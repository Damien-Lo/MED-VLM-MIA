#!/bin/bash
#SBATCH --job-name=baselines_llava_flickr_cvpr
#SBATCH --output=/sunhome/clo37/priv/MED-VLM-MIA/scripts/llava_flickr_redo_scripts/out_baselines_llava_flickr.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
# Blind-binary-classifier-style baselines for LLaVA-v1.5-7B on flickr -- mirrors
...
out_dir='/local/scratch/clo37/MED-VLM-MIA-DATA/results/ICLR_additional_results/MODERN_PIPELINE_RESULTS/llava-flickr/run0/baselines'
...
python /local/scratch/clo37/tmp_figure3_repro_compare/hulu_perf_worktree/mia/mia.py \
    ...
    job_meta_params.description="'Baselines for LLaVA-v1.5-7B members/non-members on flickr, CVPR redo'" \

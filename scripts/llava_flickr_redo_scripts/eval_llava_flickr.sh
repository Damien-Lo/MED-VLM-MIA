#!/bin/bash
#SBATCH --job-name=eval_llava_flickr_cvpr
#SBATCH --output=/sunhome/clo37/priv/MED-VLM-MIA/scripts/llava_flickr_redo_scripts/out_eval_llava_flickr.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
# metrics) for LLaVA-v1.5-7B on flickr, fresh inference -- mirrors eval_endoscopy_uniform224.sh
...
out_dir='/local/scratch/clo37/MED-VLM-MIA-DATA/results/ICLR_additional_results/MODERN_PIPELINE_RESULTS/llava-flickr/run0/eval'
...
for ((set=0; set<${#STD_SETS[@]}; set++)); do
  printf "\n>>>===================\n\nLLAVA FLICKR CVPR EVAL -- STD set $set: ${STD_SETS[$set]} \n\n=================== \n\n"

  python /local/scratch/clo37/tmp_figure3_repro_compare/hulu_perf_worktree/mia/mia.py \
      job_meta_params.test_run=false \
      job_meta_params.description="'Full-sweep evaluation for LLaVA-v1.5-7B on flickr, CVPR redo, std set: ${STD_SETS[$set]}'" \

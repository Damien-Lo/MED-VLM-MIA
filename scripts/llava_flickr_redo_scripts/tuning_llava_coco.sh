#!/bin/bash
#SBATCH --job-name=tuning_llava_coco_cvpr
#SBATCH --output=/sunhome/clo37/priv/MED-VLM-MIA/scripts/llava_flickr_redo_scripts/out_tuning_llava_coco.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
# metrics) for LLaVA-v1.5-7B on coco, fresh inference through the current/modern
...
#
# member/non-member/reference datasets: copied directly from the archive's own
# llava/coco/hyperparam_tuning/.../run_1/gn_set0/datasets/ -- unlike flickr, COCO's archive
# already ships a pre-split member_target_dataset.parquet (150 COCO-2017-IFT members) /
# non_member_target_dataset.parquet (150 COCO non-members) / reference_dataset.parquet (300,
# 50% flickr-distribution + 50% ShareGPT) for every run, and all 3 of COCO's tuning runs were
# independently verified genuine 50/50 mixture composition (no run1-anchoring workaround
# needed here, unlike flickr's mislabeled run_2/run_3).
#
...
out_dir='/local/scratch/clo37/MED-VLM-MIA-DATA/results/ICLR_additional_results/MODERN_PIPELINE_RESULTS/llava-coco/run0/tuning'
...
for ((set=0; set<${#STD_SETS[@]}; set++)); do
  printf "\n>>>===================\n\nLLAVA COCO CVPR TUNING -- STD set $set: ${STD_SETS[$set]} \n\n=================== \n\n"

  python /local/scratch/clo37/tmp_figure3_repro_compare/hulu_perf_worktree/mia/mia.py \
      job_meta_params.test_run=false \
      job_meta_params.description="'Noise-level tuning for LLaVA-v1.5-7B on coco, CVPR redo, 50/50 mixture reference (run_1-anchored), std set: ${STD_SETS[$set]}'" \

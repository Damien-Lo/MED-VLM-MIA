#!/bin/bash
#SBATCH --job-name=tuning_minigpt_flickr_cvpr
#SBATCH --output=/sunhome/clo37/priv/MED-VLM-MIA/scripts/llava_flickr_redo_scripts/out_tuning_minigpt_flickr.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
# metrics) for MiniGPT-4 on flickr, fresh inference through the current/modern
...
#
# BATCH_SIZE: UNCONFIRMED for minigpt-4 at this scale -- set conservatively. LLaVA-7B is
...
out_dir='/local/scratch/clo37/MED-VLM-MIA-DATA/results/ICLR_additional_results/MODERN_PIPELINE_RESULTS/minigpt-flickr/run0/tuning'
...
for ((set=0; set<${#STD_SETS[@]}; set++)); do
  printf "\n>>>===================\n\nMINIGPT FLICKR CVPR TUNING -- STD set $set: ${STD_SETS[$set]} \n\n=================== \n\n"

  python /local/scratch/clo37/tmp_figure3_repro_compare/hulu_perf_worktree/mia/mia.py \
      job_meta_params.test_run=false \
      job_meta_params.description="'Noise-level tuning for MiniGPT-4 on flickr, CVPR redo, 50/50 mixture reference (run_1-anchored), std set: ${STD_SETS[$set]}'" \

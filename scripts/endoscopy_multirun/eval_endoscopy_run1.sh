#!/bin/bash
#SBATCH --job-name=eval_endoscopy_run1
#SBATCH --output=/sunhome/clo37/priv/MED-VLM-MIA/scripts/endoscopy_multirun/out_eval_endoscopy_run1.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
# all 9 metrics) for endoscopy, on the uniform-224x224-resized dataset built in
...
# and per-batch compute is no longer wildly variable depending on which images
# land together, unlike the original variable-resolution dataset.
#
# Dummy descriptions ("N/A" x300): img_metrics.parts=["img"] only scores the
# image-token slice of the logits, and the conversation puts images before
# inst/desc text, so under causal attention the image-token logits can never be
# influenced by text that comes later in the sequence -- description content is
# provably irrelevant here. Real generation is skipped entirely via the existing
# pre_gen_descriptions load path (also avoids the NFS hang that step caused
...
data_dir='/local/scratch/clo37/MED-VLM-MIA-DATA/results/ICLR_additional_results/MODERN_PIPELINE_RESULTS/hulumed-endoscopy/run1'
...
for ((set=0; set<${#STD_SETS[@]}; set++)); do
  printf "\n>>>===================\n\nENDOSCOPY RUN1 EVAL -- STD set $set: ${STD_SETS[$set]} \n\n=================== \n\n"

  python /local/scratch/clo37/tmp_figure3_repro_compare/hulu_perf_worktree/mia/mia.py \
      job_meta_params.test_run=false \
      job_meta_params.description="'Full-sweep evaluation for endoscopy, uniform 224x224 resize (all images 256 vision tokens), std set: ${STD_SETS[$set]}'" \

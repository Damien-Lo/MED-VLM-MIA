#!/bin/bash
#SBATCH --job-name=7B_mia_run_v2
#SBATCH --output=out_7B_mia_run_v2.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ((run=0; run<1; run++)); do
  for ((mod=0; mod<${#modalities[@]}; mod++)); do
    out_dir=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_02_17/7B_exact_three_images
    target_dataset=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_02_17/7B_exact_three_images/datasets/target_dataset.json
    for ((set=0; set<${#STD_SETS[@]}; set++)); do
      ...
      python /home/clo37/priv/MED-VLM-MIA/mia/mia.py \
          ...
          data.pre_gen_descriptions=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_02_17/7B_exact_three_images/baselines/datasets/generated_descriptions.json \

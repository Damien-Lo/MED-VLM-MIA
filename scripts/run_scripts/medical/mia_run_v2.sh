#!/bin/bash
#SBATCH --job-name=7B_mia_run_v2_one_image
#SBATCH --output=out_7B_mia_run_v2_one_image.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ((run=0; run<1; run++)); do
  for ((mod=0; mod<${#modalities[@]}; mod++)); do
    out_dir=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_02_26/14B_exact_one_image/${modalities[$mod]}/run_0
    target_dataset=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_02_26/14B_exact_one_image/${modalities[$mod]}/run_0/datasets/target_dataset.parquet
    for ((set=17; set<${#STD_SETS[@]}; set++)); do
      printf "\n>>>===================\n\nRUNING FOR MODALILTY: ${modalities[$mod]} RUN ${run} STD $set: ${STD_SETS[$set]} \n\n=================== \n\n"
      python /home/clo37/priv/MED-VLM-MIA/mia/mia.py \
          job_meta_params.test_run=false \
          job_meta_params.description="'RUN MIA by modality on 7B Hulumed model with exactly 1 image per sample: with ${modalities[$mod]} member and nonmember data from ${nonmember_dataset[$mod]} RUN ${run} STD $set: ${STD_SETS[$set]}'" \
          ...
          data.pre_gen_descriptions=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_02_26/14B_exact_one_image/${modalities[$mod]}/run_0/baselines/datasets/generated_descriptions.json \

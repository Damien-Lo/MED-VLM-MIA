#!/bin/bash
#SBATCH --job-name=14B_mia_run
#SBATCH --output=out_14B_mia_run.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ((run=0; run<1; run++)); do
  for ((mod=0; mod<${#modalities[@]}; mod++)); do
    ...
    out_dir=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_04_25_synth_non-members/mri/run_${run}
    target_dataset=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_04_25_synth_non-members/mri/run_${run}/datasets/target_dataset.parquet
    for ((set=2; set<${#STD_SETS[@]}; set++)); do
      printf "\n>>>===================\n\nRUNING FOR MODALILTY: ${modalities[$mod]} RUN ${run} STD $set: ${STD_SETS[$set]} \n\n=================== \n\n"
      python /home/clo37/priv/MED-VLM-MIA/mia/mia.py \
          job_meta_params.test_run=false \
          job_meta_params.description="RUN MIA for 14B on ${modalities[$mod]} data with ${modalities[$mod]} nonmembers with exactly 1 image per sample" \
          ...
          data.pre_gen_descriptions=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_04_25_synth_non-members/mri/run_0/baselines/datasets/generated_descriptions.json \

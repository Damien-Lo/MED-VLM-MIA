...
for ((run=0; run<3; run++)); do
  for ((mod=0; mod<${#modalities[@]}; mod++)); do
    ...
    out_dir=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_03_29/7B_three_images/${modalities[$mod]}/run_${run}
    target_dataset=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_03_29/7B_three_images/${modalities[$mod]}/run_${run}/datasets/target_dataset.parquet
    for ((set=0; set<${#STD_SETS[@]}; set++)); do
      ...
      python /home/clo37/priv/MED-VLM-MIA/mia/mia.py \
          ...
          job_meta_params.description="RUN MIA for 7B on ${modalities[$mod]} data with ${modalities[$mod]} nonmembers with exactly 3 images per sample" \
          ...
          data.pre_gen_descriptions=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_03_29/7B_three_images/${modalities[$mod]}/run_${run}/datasets/generated_descriptions.json \

#!/bin/bash
#SBATCH --job-name=baselines_14B
#SBATCH --output=out_baselines_14B.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ((mod=0; mod<${#modalities[@]}; mod++)); do
    ...
    out_dir=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_04_25_synth_non-members/mri/run_${run}/baselines
    ...
    python /home/clo37/priv/MED-VLM-MIA/mia/mia.py \
        ...
        job_meta_params.description="Baselines for 14B on ${modalities[$mod]} data with ${modalities[$mod]} nonmembers with exactly 1 images per sample" \

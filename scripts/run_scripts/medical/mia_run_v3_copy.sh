#!/bin/bash
#SBATCH --job-name=14B_mia_run_v3
#SBATCH --output=out_14B_mia_run_v3.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ((run=0; run<1; run++)); do
  for ((mod=0; mod<${#modalities[@]}; mod++)); do
    out_dir=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_02_17/14B_exact_one_image/run_0
    target_dataset=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_02_17/14B_exact_one_image/run_0/datasets/target_dataset.parquet
    ...
    for ((set=0; set<1; set++)); do
      ...
      python /home/clo37/priv/MED-VLM-MIA/mia/mia.py \
          ...
          job_meta_params.description="'RUN MIA by modality on 14B Hulumed model with exactly 1 images per sample: with MRI member and nonmember data RUN ${run} STD $set: ${STD_SETS[$set]}'" \
          ...
          data.pre_gen_descriptions=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_02_17/14B_exact_one_image/run_0/baselines/datasets/generated_descriptions.json \

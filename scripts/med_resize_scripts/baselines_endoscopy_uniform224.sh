#!/bin/bash
#SBATCH --job-name=baselines_endoscopy_uniform224
#SBATCH --output=/sunhome/clo37/priv/MED-VLM-MIA/scripts/med_resize_scripts/out_baselines_endoscopy_uniform224.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
# Blind-binary-classifier-style baselines for endoscopy on the uniform-224x224
# dataset -- mirrors baselines_7B.sh's methodology exactly (job_type=evaluation,
...
# at the resized dataset. Real descriptions ARE generated here (unlike
# eval/tuning) since these baseline metrics aren't provably independent of
# description content the way the img-only noise-divergence metrics are.
#
...
python /local/scratch/clo37/tmp_figure3_repro_compare/hulu_perf_worktree/mia/mia.py \
    ...
    job_meta_params.description="'Baselines for endoscopy members/non-members, uniform 224x224 resize'" \

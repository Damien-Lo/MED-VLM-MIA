#!/bin/bash
#SBATCH --job-name=tuning_mri_uniform224
#SBATCH --output=/sunhome/clo37/priv/MED-VLM-MIA/scripts/med_resize_scripts/out_tuning_mri_uniform224.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
# all 9 metrics) for mri, on the uniform-224x224-resized dataset. Reference set
# is a newly-frozen 50/50 mixture (150 TCIA -- UPENN-GBM-500 + MIDI-B-Curated,
# completely disjoint from the target non-member source + 150 Prompt2MedImage,
# same distribution as the target non-members but different specific images, no
# overlap) -- mirrors the flickr experiment's p=0.5 mixture-reference structure.
# (MRI had no frozen reference set before this -- it used to resample fresh from
# the raw TCIA pool on every gn_set, the same bug endoscopy had before its
# fixedref fix.)
#
# Dummy descriptions ("N/A" x600, target+reference): same img-only rationale as
# eval_mri_uniform224.sh.
#
...
for ((set=0; set<${#STD_SETS[@]}; set++)); do
  printf "\n>>>===================\n\nMRI UNIFORM224 TUNING -- STD set $set: ${STD_SETS[$set]} \n\n=================== \n\n"

  python /local/scratch/clo37/tmp_figure3_repro_compare/hulu_perf_worktree/mia/mia.py \
      job_meta_params.test_run=false \
      job_meta_params.description="'Noise-level tuning for mri, uniform 224x224 resize (all images 256 vision tokens), 50/50 mixture reference, std set: ${STD_SETS[$set]}'" \

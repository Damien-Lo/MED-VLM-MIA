#!/bin/bash
#SBATCH --job-name=flickr_llava_mixture_sweep_v4
#SBATCH --output=out_flickr_llava_mixture_sweep_v4.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
# Fourth attempt at the mixture sweep -- see build_mixture_reference_sets_v4_run1_anchored.py's
# docstring and /local/scratch/clo37/MED-VLM-MIA-DATA/results/ICLR_additional_results/README.txt
# for the full history. Unlike v3, BOTH the target set and the p=0.5 reference set are now
# byte-identical to run_1's real, validated data -- so p=050 here is guaranteed to reproduce the
# published Figure 3 exactly (both "Estimated" and "True optimal" curves), and every other p
# nests around that same anchor (subsetting/supplementing run_1's own 150-flickr/150-sharegpt
# reference core) rather than being independently resampled.
#
# p=050 runs FIRST as the sanity check, then the rest of the sweep continues.
...
for pp in "${MIXTURE_PS[@]}"; do
  ...
  for ((set=0; set<${#STD_SETS[@]}; set++)); do
    printf "\n>>>===================\n\nMIXTURE p=0.%s (v4 run_1-anchored) -- STD set $set: ${STD_SETS[$set]} \n\n=================== \n\n" "${pp}"
    ...
    python /home/clo37/priv/MED-VLM-MIA/mia/mia.py \
        ...
        job_meta_params.description="'Flickr/LLaVA mixture-sweep rerun (v4, run_1-anchored): reference = ${pp}% flickr non-members (run_1-anchored) / $((100 - 10#$pp))% ShareGPT, std set: ${STD_SETS[$set]}'" \

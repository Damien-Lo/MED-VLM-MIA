#!/bin/bash
#SBATCH --job-name=mia_7B_hard
#SBATCH --output=out_mia_7B_hard.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ((run=0; run<1; run++)); do
  ...
  out_dir=/local/scratch/clo37/MED-VLM-MIA-DATA/results/2026_07_29_original_mia_on_roco_with_hard_BBC_filter/run_${run}
  ...
  for ((set=7; set<${#STD_SETS[@]}; set++)); do
    printf "\n>>>===================\n\nRUNING FOR MODALILTY: STD $set: ${STD_SETS[$set]} \n\n=================== \n\n"
    python /home/clo37/priv/MED-VLM-MIA/mia/mia.py \
        job_meta_params.test_run=false \
        job_meta_params.description="Baselines for roco members and synthetic non-members that pass the hard BBC filter" \

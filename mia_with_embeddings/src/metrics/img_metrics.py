...
from src.metrics.proposed_metrics import cross_entropy_mink, cross_entropy_diff_mink, renyi_kl_div_maxk, renyi_divergence_maxk, renyi_kl_div_ripple_maxk, renyi_divergence_ripple_maxk, entropy_gated_kl_div_mink
import sys
...
def get_img_metric_by_parts(meta_metrics, cfg):
    ...
    #=====================================
    #ENTROPY GATED KL DIVERGENCE METRICS
    #=====================================
    if "entropy_gated_no_norn_kl_div" in cfg.img_metrics.metrics_to_use:
        if 'kld_metrics' not in pred:
            pred['kld_metrics'] = dict()
        if 'kld_metrics' not in meta:
            meta['kld_metrics'] = dict()
        (pred['kld_metrics']["entropy_gated_no_norn_kl_div"],
         meta['kld_metrics']["entropy_gated_no_norn_kl_div_tkn_vals"],
         meta['kld_metrics']["entropy_gated_no_norn_kl_div_tkn_order"]) = entropy_gated_kl_div_mink(
            meta_metrics['no_norm_probs'], meta_metrics['entropies'], cfg.img_metrics.entropy_gated_no_norn_kl_div, cfg)

    if "entropy_gated_renyi_05_kl_div" in cfg.img_metrics.metrics_to_use:
        if 'kld_metrics' not in pred:
            pred['kld_metrics'] = dict()
        if 'kld_metrics' not in meta:
            meta['kld_metrics'] = dict()
        (pred['kld_metrics']["entropy_gated_renyi_05_kl_div"],
         meta['kld_metrics']["entropy_gated_renyi_05_kl_div_tkn_vals"],
         meta['kld_metrics']["entropy_gated_renyi_05_kl_div_tkn_order"]) = entropy_gated_kl_div_mink(
            meta_metrics['renyi_05_probs'], meta_metrics['entropies'], cfg.img_metrics.entropy_gated_renyi_05_kl_div, cfg)

    if "entropy_gated_renyi_1_kl_div" in cfg.img_metrics.metrics_to_use:
        if 'kld_metrics' not in pred:
            pred['kld_metrics'] = dict()
        if 'kld_metrics' not in meta:
            meta['kld_metrics'] = dict()
        (pred['kld_metrics']["entropy_gated_renyi_1_kl_div"],
         meta['kld_metrics']["entropy_gated_renyi_1_kl_div_tkn_vals"],
         meta['kld_metrics']["entropy_gated_renyi_1_kl_div_tkn_order"]) = entropy_gated_kl_div_mink(
            meta_metrics['renyi_1_probs'], meta_metrics['entropies'], cfg.img_metrics.entropy_gated_renyi_1_kl_div, cfg)

    if "entropy_gated_renyi_2_kl_div" in cfg.img_metrics.metrics_to_use:
        if 'kld_metrics' not in pred:
            pred['kld_metrics'] = dict()
        if 'kld_metrics' not in meta:
            meta['kld_metrics'] = dict()
        (pred['kld_metrics']["entropy_gated_renyi_2_kl_div"],
         meta['kld_metrics']["entropy_gated_renyi_2_kl_div_tkn_vals"],
         meta['kld_metrics']["entropy_gated_renyi_2_kl_div_tkn_order"]) = entropy_gated_kl_div_mink(
            meta_metrics['renyi_2_probs'], meta_metrics['entropies'], cfg.img_metrics.entropy_gated_renyi_2_kl_div, cfg)

    if "entropy_gated_renyi_inf_kl_div" in cfg.img_metrics.metrics_to_use:
        if 'kld_metrics' not in pred:
            pred['kld_metrics'] = dict()
        if 'kld_metrics' not in meta:
            meta['kld_metrics'] = dict()
        (pred['kld_metrics']["entropy_gated_renyi_inf_kl_div"],
         meta['kld_metrics']["entropy_gated_renyi_inf_kl_div_tkn_vals"],
         meta['kld_metrics']["entropy_gated_renyi_inf_kl_div_tkn_order"]) = entropy_gated_kl_div_mink(
            meta_metrics['renyi_inf_probs'], meta_metrics['entropies'], cfg.img_metrics.entropy_gated_renyi_inf_kl_div, cfg)

...
def entropy_gated_kl_div_mink(renyi_probs, orig_entropies, metric_cfg, cfg, eps=1e-12):
    """
    Entropy-gated variant of renyi_kl_div_maxk.

    Same per-token KL-divergence computation, but instead of scoring on the top-k
    HIGHEST-divergence tokens, gates the token selection on the ORIGINAL (unperturbed)
    input's per-token entropy: picks the k LOWEST-entropy positions -- the tokens the
    model is most confident about on the real image, i.e. its "deep member well"
    tokens -- and scores on the average divergence at exactly those positions.
    Selection is always driven by the original input's entropy, never the augmented
    one, since a "well" is a property of what the model has encoded about the sample
    itself, not of the perturbation applied to probe it.

    Returns (result, meta_tkn_vals, meta_tkn_order):
        result:         same shape/semantics as renyi_kl_div_maxk's first return value
        meta_tkn_vals:  same shape as renyi_kl_div_maxk's meta (full per-token kld arrays)
        meta_tkn_order: {aug: {setting: [sample][token indices, ascending by original
                        entropy]}}. Selection doesn't depend on aug/setting (always
                        gated on the original input), so the same per-sample order list
                        is duplicated across aug/setting keys purely to match the shape
                        the rest of the pipeline (get_proc_meta_metrics / proc_meta.json)
                        expects. Any ratio's min-k selection is a prefix slice of this list.
    """
    result = dict()
    meta_tkn_vals = dict()
    meta_tkn_order = dict()
    ...
    original_probs = renyi_probs['orig'][0]
    orig_entropy_per_sample = orig_entropies['orig'][0]

    # Ascending-entropy token order per sample: lowest entropy (deepest "member well")
    # first. Independent of augmentation/setting -- always gated on the original input.
    entropy_order_per_sample = [np.argsort(sample_entropies).tolist() for sample_entropies in orig_entropy_per_sample]
    ...
    for aug, settings in renyi_probs.items():
        ...
        all_settings_in_aug = dict()                                   #Shape: {setting: [sample, kld]}
        order_settings_in_aug = dict()                                  #Shape: {setting: [sample, token_order]}
        ...
        for setting_idx, setting in enumerate(settings):
            ...
            all_settings_in_aug[str(key)] = all_samples_in_setting_values
            order_settings_in_aug[str(key)] = entropy_order_per_sample
            # all_settings_in_aug {setting: [sample, kld(1D)]}

        # For Aug, Push sampled raw kl_array/token_order to meta return
        meta_tkn_vals[aug] = all_settings_in_aug
        meta_tkn_order[aug] = order_settings_in_aug
        ...
        if 'none' in setting_version_accumilator:
            for _ratio in ratio:
                for setting_name, setting_values in all_settings_in_aug.items():
                    ...
                    sample_scores = list()
                    for sample_idx, sample in enumerate(setting_values):
                        k_length = max(1, int(_ratio * len(sample)))
                        selected = entropy_order_per_sample[sample_idx][:k_length]
                        # Members have lower kld at their deep-well tokens, so flip the
                        # score (same convention as the max_k metrics) so higher = more
                        # member-like.
                        sample_scores.append(float(-1 * np.mean(np.asarray(sample)[selected])))
    ...
    for _ratio in ratio:
        for combination, samples in final_combinations.items():
            key = f"Entropy_gated_{_ratio}_{metric_cfg.suffix}_kld_{combination}"
            sample_scores = list()
            for sample_idx, sample in enumerate(samples):
                ...
                sample_scores.append(float(-1 * np.mean(np.asarray(sample)[selected])))
            result['aggregated'][key] = sample_scores
    return result, meta_tkn_vals, meta_tkn_order

...
def mod_infer_batch_hulu(model, batch, tokenizer, vis_processor, parts, use_augmentation):
    ...
    if use_augmentation:
        ...
        total_vision_embeddings = {
            "orig": list()
        }

        has_vision_embeddings = hasattr(model, "get_last_vision_encoder_embeddings")
        ...
        for sample_images, sample_aug_images, sample_inst, sample_desc in zip(images_of_batch, aug_images_of_batch, insts_of_batch, descs_of_batch):            
            ...
            if has_vision_embeddings:
                vision_emb = model.get_last_vision_encoder_embeddings()
                total_vision_embeddings["orig"].append(
                    vision_emb.detach().cpu() if vision_emb is not None else None
                )
            else:
                total_vision_embeddings["orig"].append(None)
            ...
            for aug_name, aug_matrix in sample_aug_images.items():
                ...
                if aug_name not in total_vision_embeddings:
                    total_vision_embeddings[aug_name] = [list() for _ in range(len(aug_matrix))]
                for setting_idx, setting_images in enumerate(aug_matrix):
                    ...
                    if has_vision_embeddings:
                        vision_emb = model.get_last_vision_encoder_embeddings()
                        total_vision_embeddings[aug_name][setting_idx].append(
                            vision_emb.detach().cpu() if vision_emb is not None else None
                        )
                    else:
                        total_vision_embeddings[aug_name][setting_idx].append(None)
        ...
        return total_parts, total_token_labels, total_vision_embeddings
